...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    { icon: '💰', label: isAr ? 'التقارير المالية'  : 'Financial Reports', key: 'financial', sub: [
      ...
      { label: isAr ? 'بوب-أب الكوبون'   : 'Coupon Popup',      path: '/popup-coupon'   },
    ...
    { icon: '💹', label: isAr ? 'المالية والعمليات' : 'Finance & Ops',     key: 'finance', sub: [
      ...
      { label: isAr ? 'وصفات الوجبات'     : 'Meal Recipes',      path: '/meal-recipes'  },
      { label: isAr ? 'الإنتاج اليومى'    : 'Daily Production',  path: '/production'    },
      { label: isAr ? 'تخطيط المشتريات'   : 'Procurement Plan',  path: '/procurement-planning' },
      { label: isAr ? 'تحليل التكاليف'    : 'Cost Analysis',     path: '/cost-analysis' },
      { label: isAr ? 'قائمة الدخل (P&L)' : 'Profit & Loss',     path: '/profit-loss'   },
