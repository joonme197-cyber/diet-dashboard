...
import {
  getClientSubscriptions, addSubscription, updateSubscription,
  freezeDay, unFreezeDay, addBonusDays, addPayment,
  getSubscriptionStatus, getStatusLabel
...
export default function ClientProfile() {
  ...
  const handleAction = (action) => {
    ...
    if (action === 'meals') {
      ...
      setRenewType('same');
      setSubForm(f => ({...f, startDate: '', durationWeeks: active?.durationWeeks||4}));
      setShowRenew(true);
  ...
  const handleRenew = async () => {
    const active = subscriptions.find(s => getSubscriptionStatus(s) === 'active');
    const startDate = subForm.startDate || new Date().toISOString().split('T')[0];
  ...
  const renderCalendar = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push({ d, dateStr, isFrozen: frozen.includes(dateStr), isInRange: dateStr >= selectedSub.startDate && dateStr <= selectedSub.endDate, isToday: dateStr === today });
    }
    return (
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          ...
          <strong>{calendarMonth.toLocaleDateString('ar', { month: 'long', year: 'numeric' })}</strong>
          <button className="btn btn-ghost btn-sm" onClick={() => setCalendarMonth(p => new Date(p.getFullYear(), p.getMonth()+1))}>←</button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
          {days.map((day, i) => (
            <div key={i}>
              {day ? (
                <button onClick={() => day.isInRange && handleFreezeDay(day.dateStr)}
                  style={{
                    ...
                    background: day.isFrozen ? '#fef3c7' : day.isToday ? '#0d9488' : day.isInRange ? '#f0fdfa' : '#f8fafc',
                    color: day.isFrozen ? '#d97706' : day.isToday ? 'white' : day.isInRange ? '#0f766e' : '#cbd5e1',
                    border: day.isFrozen ? '1.5px solid #fde68a' : day.isInRange ? '1px solid #ccfbf1' : '1px solid transparent',
                  }}
                >
                  {day.d}
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '10px', fontSize: '0.75rem', color: '#64748b' }}>
          <span>🟩 في الاشتراك</span>
          <span>🟨 مجمد ❄</span>
          <span>🟦 اليوم</span>
          <span style={{ marginRight: 'auto', fontWeight: 700, color: '#d97706' }}>
            {(selectedSub.frozenDays||[]).length} يوم مجمد
          </span>
        </div>
      </div>
    );
                        ...
                        {MEAL_SECTIONS.map(sec => {
                          ...
                          return (
                            <div key={sec.key} style={{ marginBottom: '12px' }}>
                              <div style={{ fontSize: '0.78rem', fontWeight: 700, color: '#64748b', borderBottom: '1px solid #e2e8f0', paddingBottom: '4px', marginBottom: '6px' }}>
                                {sec.icon} {sec.label}
                              </div>
                              {meals.map(meal => (
                                <label key={meal.id} style={{
                                  ...
                                  background: isMealSelected(meal) ? '#f0fdfa' : 'transparent',
                                  border: isMealSelected(meal) ? '1px solid #ccfbf1' : '1px solid transparent',
                                  marginBottom: '3px', transition: 'all 0.15s',
                                }}>
                                  <input type="checkbox" checked={isMealSelected(meal)}
