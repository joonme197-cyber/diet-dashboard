...
export default function LabelsPage() {
  ...
  const fetchAllMeals = async () => {
    ...
    setSelected(result.filter(c => c.hasMeals).map(c => c.id));
  ...
  const selectWithMeals = () => setSelected(clientsWithMeals.filter(c => c.hasMeals).map(c => c.id));
          ...
          <div>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'16px' }}>
              {selectedClients.map(c => (
                <StickerLabel key={c.id} client={c} deliveryDate={new Date(selectedDate)}
                  dailyMeals={formatMeals(c.dailyMeals)} lang={lang} />
              ))}
            </div>
          </div>
        )}
