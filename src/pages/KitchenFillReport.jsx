...
export default function KitchenFillReport() {
  ...
  const buildReport = async () => {
    ...
    for (const client of clientsList) {
      ...
      if (!activeSub) continue; // فقط النشطين
      clientMap[client.id] = { client, activeSub };
    }
    ...
    for (const dayDoc of dayDocs) {
      ...
      if (!entry) continue; // تجاهل غير النشطين
      ...
      for (const sec of MEAL_SECTIONS) {
        ...
        for (const m of mealList) {
          const master = MEALS_DATA.find(x => x.id === m.id);
          clientMeals.push({
            section:  isAr ? sec.labelAr : sec.labelEn,
            mealName: isAr ? (master?.mealTitle || m.title || m.id) : (master?.mealTitleEn || master?.mealTitle || m.title || m.id),
          });
        }
      }
      ...
      {fetched && rows.length > 0 && (
        <div className="print-area" style={{ padding:'0 32px' }}>
          ...
          <table style={{
            ...
            fontSize:'12px', direction: isAr ? 'rtl' : 'ltr',
          }}>
            <thead>
                ...
                row.meals.map((meal, mi) => (
                  <tr key={`${row.clientId}-${mi}`}
                    ...
                    {mi === 0 && (
                      <td rowSpan={row.meals.length} style={{
                        ...td,
                        textAlign:'center', fontWeight:700, color:'#0d9488',
                        verticalAlign:'middle',
                        background: ri % 2 === 0 ? '#f0fdfa' : '#e0f7f4',
                      }}>
                        {row.clientCode}
                    ...
                    {/* البروتين والكارب فقط في الصف الأول */}
                    {mi === 0 && (
                      <td rowSpan={row.meals.length} style={{
